...
class TestProjectValidation:
    ...
    def test_validate_detects_invalid(self, project_object,
                                      schema_invalid_file_path):
        with pytest.raises(ValidationError):
            validate_project(project=project_object,
                             schema=schema_invalid_file_path)

    def test_validate_detects_invalid_imports(self, project_object,
                                              schema_imports_file_path):
        with pytest.raises(ValidationError):
            validate_project(project=project_object,
                             schema=schema_imports_file_path)
    ...
    def test_validate_works_with_dict_schema(self, project_object,
                                             schema_file_path):
        validate_project(project=project_object,
                         schema=_load_yaml(schema_file_path))
...
class TestSampleValidation:
    ...
    def test_validate_works(self, project_object, sample_name,
                            schema_samples_file_path):
        validate_sample(project=project_object, sample_name=sample_name,
                        schema=schema_samples_file_path)
    ...
    @pytest.mark.parametrize("sample_name", [0, 1, "GSM1558746"])
    def test_validate_detects_invalid(self, project_object, sample_name,
...
class TestConfigValidation:
    def test_validate_succeeds_on_invalid_sample(
        ...
        validate_config(project=project_object,
                        schema=schema_sample_invalid_file_path)


class TestRemoteValidation:
    @pytest.mark.parametrize("schema_url",
                             ["https://schema.databio.org/pep/2.0.0.yaml"])
    def test_validate_works_with_remote_schemas(self,
                                                project_object, schema_url):
        validate_project(project=project_object, schema=schema_url)
        validate_config(project=project_object, schema=schema_url)
        validate_sample(project=project_object, schema=schema_url,
                        sample_name=0)
